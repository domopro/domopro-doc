...
Il faut regarder le fichier /usr/share/nginx/www/domopro/log/nginx.error, celui-ci indique l'erreur, 
...
echo "DROP USER 'domopro'@'localhost'" | mysql -uroot -p
echo "CREATE USER 'domopro'@'localhost' IDENTIFIED BY '${bdd_password}';" | mysql -uroot -p
echo "GRANT ALL PRIVILEGES ON domopro.* TO 'domopro'@'localhost';" | mysql -uroot -p
cd /usr/share/nginx/www/domopro
...
J'ai des {{...}} partout::
La cause la plus frequente est l'utilisation d'un plugin en version beta, et pas domopro, ou l'inverse

''''

Lors d'une commande j'ai une roue qui tourne sans s'arreter::
Encore une fois cela est souvent dû à un plugin en beta alors que domopro est en stable
...
Je n'ai plus accès a domopro ni en SSH::
Cette erreur n'est pas dû à domopro mais à un probleme avec le système. 
...
En ssh faite "sudo service domopro restart" pour forcer un redémarrage de celui-ci. Si ca ne marche pas faites : 

[source,bash]
sudo su -
rm -rf /usr/share/nginx/www/domopro/core/nodeJS
reboot

Puis mettre à jour domopro, le redemarrer à nouveau et ca devrait être bon
...
Vérifiez que vous avez bien mis les /domopro si vous utilisez ceux-ci juste après l'IP de domopro 
...
* probleme de corruption de fichier, arrive souvent suite à un arrêt non propre de domopro (coupure de courant)

Malheureusement il n'y a pas beaucoup de solution si c'est le deuxieme cas, 
le mieux étant de récupérer un backup (disponible dans /usr/share/nginx/www/domopro/backup par défaut), 
de réinstaller domopro et de restaurer le backup.
