...
Page permettant d'interconnecter plusieurs domopro en mode maitre/esclave. Cette possibilité permet par exemple de déporter le zwave, le rfxcom, l'enOcean ou le GSM sur d'autre raspberry pi ou domopro mini. Vous pourrez aussi à partir de cette page avoir un status de vos domopro esclave, consulter leur log, les mettres à jour, les arrêter...

== Le réseaux domopro

Accessible depuis Général -> Administration -> Réseaux domopro :
...
* *Complement* : si vous avez une mini, center ou pro vous n'avez rien a mettre sinon vous devez mettre /domopro
* *Port* : 80 si vous n'avez pas modifier la configuration nginx

Ensuite, toujours sur le maitre aller sur Générale -> Administration -> Réseaux domopro et la faites "Ajouter un domopro" : 

image::../images/jeeNetwork.JPG[]

- donné lui un nom
- indiquer l'IP de l'esclave
- indiquer le complement si nécessaire (ex : /domopro si vous n'avez une mini/center/pro et que vous utilisez le /domopro pour vous connecter sur l'esclave)
