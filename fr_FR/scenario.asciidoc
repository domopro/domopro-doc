...
Lors de la création domopro va vous demander le type de scénario à faire : 
...
* *A* : permet de dire à domopro de lancer les actions action du bloc A à une heure donnée (sous la forme hhmm). Ce bloc est non bloquant
...
Une fois la commande trouvé domopro vous demande ce que vous voulez tester : 
...
[TIP]
Si vous cliquez sur "Ne rien mettre" domopro va juste écrire la commande dans le champs condition en vous laissant la main pour la suite
...
* *\#start#*  : déclenché au (re)démarrage de domopro,
...
* *\#IP#* : IP interne de domopro,
* *\#hostname#* : nom de la machine domopro,
...
* *say* : permet de faire dire un texte à domopro (ne marche que si un onglet domopro est ouvert dans le navigateur)
...
Fonctionalité permettant de transformer un scénario en template pour par exemple l'appliquer sur un autre domopro ou le partager sur le market. C'est aussi a partir de la que vous pouvez récuperer un scénario du market
...
* D'envoyer un template à domopro (fichier JSON prealablement recuperé)
...
* Consulter les templates actuellement présent sur votre domopro
...
* *Télécharger* : permet de recuperer le template sous forme de fichier JSON pour le renvoyer sur un autre domopro par exemple

En dessous vous avez la partie pour appliquer votre template au scénario courant.

[TIP]
Etant donnée que d'un domopro à l'autre ou d'une installation à une autre les commandes peuvent etre differente domopro vous demande la correspondance des commandes entre celle résente lors de la création du template et celle presente chez vous
...
En haut vous pouvez rafraichir le log, le télécharger ou le supprimer. La taille des logs n'est pas limité en execution mais en nombre de ligne (en fonction de la valeur mise dans la configuration de domopro)
